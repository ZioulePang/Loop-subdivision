...
RenderController::RenderController
    ...
    QObject::connect(   renderWindow->vertexVolumnSlider,	            SIGNAL(valueChanged(int)),
                        this,                                       SLOT(vertexVolumnChanged(int)));
    ...
    QObject::connect(   renderWindow->writeNewFiles, 		        SIGNAL(stateChanged(int)),
                        this,                                       SLOT(writeAFiles(int)));
...
void RenderController::writeAFile(int state)
{
    renderParameters->writeFiles = (state == Qt::Checked);

    // dump the file to out
    std::ofstream fileout;
    fileout.open("D:\Assignment\Geometry\LoopSubdivisionRelease.tar\LoopSubdivisionRelease\LoopSubdivisionRelease\newText\text.txt");
    directedEdgeSurface->WriteObjectStream(fileout);
...
void RenderController::vertexVolumnChanged(int value)
    { // RenderController::vertexVolumnChanged()
    // reset the model's y translation (slider ticks are 1/512 each)
    renderParameters->vertexVolumn = (int) value;
